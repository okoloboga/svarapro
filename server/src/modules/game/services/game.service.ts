...
export class GameService {
  ...
  async startAntePhase(roomId: string): Promise<void> {
    ...
    const { updatedGameState, actions } = this.bettingService.processAnte(gameState, gameState.minBet);
    ...
    const phaseResult = this.gameStateService.moveToNextPhase(gameState, 'blind_betting');
    gameState = phaseResult.updatedGameState;
    gameState.log.push(...phaseResult.actions);

    gameState.currentPlayerIndex = this.playerService.findNextActivePlayer(gameState.players, gameState.dealerIndex);
  ...
  async processAction(
    ...
    const { canPerform, error } = this.bettingService.canPerformAction(gameState.players[playerIndex], action, gameState);
    if (!canPerform) {
    ...
    switch (action) {
      ...
      case 'blind_bet':
      case 'look':
        return this.processBlindBettingAction(roomId, gameState, playerIndex, action, amount);
      case 'call':
      case 'raise':
        return this.processBettingAction(roomId, gameState, playerIndex, action, amount);
      default:
        return { success: false, error: 'Недопустимое действие в текущей фазе' };
    }
  }

  private async handleFold(roomId: string, gameState: GameState, playerIndex: number): Promise<GameActionResult> {
    const player = gameState.players[playerIndex];
    gameState.players[playerIndex] = this.playerService.updatePlayerStatus(player, { hasFolded: true, isActive: false, lastAction: 'fold' });
  ...
  private async processBlindBettingAction(
    ...
    switch (action) {
      case 'blind_bet': {
        ...
        const { updatedPlayer, action: blindAction } = this.playerService.processPlayerBet(player, blindBetAmount, 'blind_bet');
  ...
  private async processBettingAction(
    ...
    switch (action) {
      case 'call': {
        ...
        const { updatedPlayer, action: callAction } = this.playerService.processPlayerBet(player, callAmount, 'call');
        gameState.players[playerIndex] = updatedPlayer;
      ...
      case 'raise': {
        if (!amount || amount <= gameState.currentBet) {
          return { success: false, error: `Повышение должно быть больше текущей ставки ${gameState.currentBet}` };
        }
        const raiseAmount = amount - player.currentBet;
        if (player.balance < raiseAmount) {
          return { success: false, error: 'Недостаточно средств' };
        }
        const { updatedPlayer, action: raiseAction } = this.playerService.processPlayerBet(player, raiseAmount, 'raise');
        gameState.players[playerIndex] = updatedPlayer;
  ...
  private async endBettingRound(roomId: string, gameState: GameState): Promise<void> {
    ...
    if (winners.length > 1) {
      ...
      const svaraAction: GameAction = { type: 'svara', telegramId: 'system', timestamp: Date.now(), message: 'Объявлена "Свара"!' };
      gameState.log.push(svaraAction);
      await this.redisService.setGameState(roomId, gameState);
      await this.redisService.publishGameUpdate(roomId, gameState);
      await this.startSvaraGame(roomId, winners.map((w) => w.id));
  ...
  private async startSvaraGame(roomId: string, winnerIds: string[]): Promise<void> {
    let gameState = await this.redisService.getGameState(roomId);
    if (!gameState) return;

    const { updatedGameState, actions } = this.gameStateService.initializeSvaraGame(gameState, winnerIds);
    gameState = updatedGameState;
    gameState.log.push(...actions);
  ...
  private async endGameWithWinner(roomId: string, winnerId: string): Promise<void> {
    let gameState = await this.redisService.getGameState(roomId);
    if (!gameState) return;

    const { updatedGameState, actions } = this.bettingService.processWinnings(gameState, [winnerId]);
    gameState = updatedGameState;
    gameState.log.push(...actions);

    const phaseResult = this.gameStateService.moveToNextPhase(gameState, 'finished');
    gameState = phaseResult.updatedGameState;
    gameState.log.push(...phaseResult.actions);
  ...
  private async endGame(roomId: string): Promise<void> {
    ...
    const action: GameAction = { type: 'join', telegramId: 'system', timestamp: Date.now(), message: 'Игра завершена без победителя' };
    gameState.log.push(action);
