...
export class GameService {
  ...
  async leaveRoom(roomId: string, userId: string): Promise<void> {
    ...
    if (room.players.length === 0) {
      ...
      if (room.status === 'playing') {
        await this.markPlayerInactive(roomId, userId);
      }
    }
    
    // Удаляем связь игрока с комнатой
    await this.redisService.removePlayerFromRoom(roomId, userId);
  }

  // Присоединиться к игре
  async joinGame(roomId: string, userId: string, userData: any): Promise<GameActionResult> {
    const room = await this.redisService.getRoom(roomId);
    if (!room) {
    ...
    await this.redisService.setRoom(roomId, room);
    await this.redisService.addPlayerToRoom(roomId, userId);
    await this.redisService.publishRoomUpdate(roomId, room);
  ...
  async startAntePhase(roomId: string): Promise<void> {
    ...
    // Раздаем карты
    const dealResult = this.gameStateService.dealCardsToPlayers(updatedGameState);
    gameState.log.push(...dealResult.actions);

    // Переходим к фазе ставок вслепую
    const phaseResult = this.gameStateService.moveToNextPhase(dealResult.updatedGameState, 'blind_betting');
  ...
  async processAction(
    ...
    switch (gameState.status) {
      case 'blind_betting':
        return await this.processBlindBettingAction(roomId, gameState, playerIndex, action, amount);
      case 'betting':
        return await this.processBettingAction(roomId, gameState, playerIndex, action, amount);
      default:
    ...
    switch (action) {
      case 'blind_bet':
        ...
        if (!amount || amount < gameState.lastBlindBet * 2) {
          return { 
            success: false, 
            error: `Минимальная ставка вслепую: ${gameState.lastBlindBet * 2 || gameState.minBet}` 
          };
        }
        ...
        // Делаем ставку
        const { updatedPlayer, action: blindAction } = this.playerService.processPlayerBet(
          player, 
          amount, 
          'blind_bet'
        );
  ...
  private async processBettingAction(
    ...
    switch (action) {
      case 'call':
        ...
        // Уравниваем ставку
        const { updatedPlayer: callPlayer, action: callAction } = this.playerService.processPlayerBet(
          player, 
          callAmount, 
          'call'
        );
      ...
      case 'raise':
        ...
        // Повышаем ставку
        const { updatedPlayer: raisePlayer, action: raiseAction } = this.playerService.processPlayerBet(
          player, 
          raiseAmount, 
          'raise'
        );
  ...
  private async startSvaraGame(roomId: string, winnerIds: string[]): Promise<void> {
    ...
    const { updatedGameState, actions } = this.gameStateService.initializeSvaraGame(
      gameState, 
      winnerIds
    );
    ...
    // Устанавливаем текущего игрока (следующий после дилера)
    updatedGameState.currentPlayerIndex = this.playerService.findNextActivePlayer(
  ...
  private async endGameWithWinner(roomId: string, winnerId: string): Promise<void> {
    ...
    const phaseResult = this.gameStateService.moveToNextPhase(updatedGameState, 'finished');
    updatedGameState.log.push(...phaseResult.actions);
  ...
  async markPlayerInactive(roomId: string, userId: string): Promise<void> {
    const gameState = await this.redisService.getGameState(roomId);
    if (!gameState) {
      return;
    }
