...
export class AdminsController {
  ...
  async createPassword(
    ...
    if (!passwordRegex.test(body.password)) {
      throw new BadRequestException('Password must contain only letters and numbers (6-20 characters)');
    }

    const admin = await this.adminsService.createPassword(body.telegramId, body.password);
  ...
  async verifyPassword(
    ...
    const isValid = await this.adminsService.verifyPassword(body.telegramId, body.password);
    return { isValid };
  }
