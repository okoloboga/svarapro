...
export class ApiService {
  ...
  constructor() {
    if (!process.env.EXNODE_API_SECRET || !this.apiPublic) {
      throw new BadRequestException('EXNODE_API_SECRET or EXNODE_API_PUBLIC is not defined in environment variables');
    }
  }
  ...
  async createDepositAddress(token: string, clientTransactionId: string): Promise<{ address: string; trackerId: string }> {
    ...
    const response = await axios.post(`${this.baseUrl}/api/transaction/create/in`, body, {
      headers: {
  ...
  async getTransactionStatus(trackerId: string): Promise<{ status: string; amount?: number; transaction_hash?: string | undefined }> {
    const timestamp = Math.floor(Date.now() / 1000).toString();
    const signature = createHmac('sha512', this.getApiSecret())
      .update(timestamp + trackerId)
      .digest('hex');

    const response = await axios.get(`${this.baseUrl}/transaction/get?tracker_id=${trackerId}`, {
      headers: {
        ApiPublic: this.apiPublic!,
        Signature: signature,
        Timestamp: timestamp,
